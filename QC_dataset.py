...
def header_contains_filter_strings(header, filter_strings):
    ...
    for filter_string in filter_strings:
        ...
        if filter_string_lower not in header_lower:
            return False  # Found a filter string not in header
    return True  # All filter strings found in header
    ...
    for i in range(len(values)):
        for j in range(i+1, len(values)):
            if values[i] == values[j]:
                if keys[i][:4] in refseq_leaders:
                    if not keys[i] in return_dict:
                        return_dict[keys[i]] = values[i]
                elif keys[j][:4] in refseq_leaders:
                    if not keys[i] in return_dict:
                        return_dict[keys[j]] = values[j]
    return return_dict

def parse_arguments(a=None):
    ...
    parser.add_argument('-r', '--refseq_only', required=False,default=False, action='store_true', help='keep only refseq type headers in output')
...
def main(a):
    ...
    if n_filter:
        ...
        if isinstance(n_content_value, int):
            if input_file_type == "fasta" or input_file_type == "fastq":
                for header, sequence in seq_dict.items():
                    # Check if sequence contains 'N' repeated at least n_filter times
                    if 'N' * n_content_value not in sequence:
                        filtered_dict[header] = sequence
                
            elif input_file_type == "directory" or input_file_type == "zip":
    
                for header, sequences in seq_dict.items():
                    # Filter each sequence in the list based on the criterion
                    filtered_sequences = [seq for seq in sequences if 'N' * n_content_value not in seq]
            ...
            if input_file_type == "fasta" or input_file_type == "fastq":
                if input_file_type == "fasta" or input_file_type == "fastq":
                    for header, sequence in seq_dict.items():
                        # Check if sequence contains 'N' repeated at least n_filter times
                        count_n = sequence.upper().count('N')  # Count 'N' and 'n', making it case-insensitive
                        total_length = len(sequence)
                        percent_n = (count_n / total_length)
                        if percent_n < n_content_value:
                            filtered_dict[header] = sequence
    ...
    if args.gc_lower > 0.0 or args.gc_upper < 1.0:
        ...
        if input_file_type == "fasta" or input_file_type == "fastq":
            for header, sequence in seq_dict.items():
                # Check if sequence contains 'N' repeated at least n_filter times
                if args.gc_lower <= calculate_gc_content(sequence) <= args.gc_upper:
                    filtered_dict[header] = sequence
    ...
    if args.strings_to_include:
        ...
        filtered_dict = {header: seq_dict[header] for header in seq_dict if header_contains_filter_strings(header, args.strings_to_include)}
    ...
    if args.strings_to_exclude:
        ...
        filtered_dict = {header: seq_dict[header] for header in seq_dict if not header_contains_filter_strings(header, args.strings_to_exclude)}
    ...
    if args.save_drops:
        ...
        for key in removed_keys:
            ...
            seq_record = SeqRecord(Seq(sequence[0]), id=key, description="Removed")
            seq_records.append(seq_record)
    ...
    return(original_seq_dict,seq_dict)
